...
# Compatibility wrapper for quantum key generation
def get_quantum_key_compatible(length=32):
    """Get quantum key compatible with crypto_utils"""
    try:
        key_bytes = bb84_shared_key(length)
        # Your qkd function returns bytes, ensure we have the right format for crypto_utils
        ...
        print(f"Quantum key generation failed: {e}, using fallback")
        # Secure fallback
...
def home():
    return jsonify({
        "system": "Quantum-Blockchain Voting API",
        "version": "3.1",
        "timestamp": datetime.now().isoformat(),
...
def cast_vote():
    ...
    try:
        # Use the compatibility wrapper for quantum key generation
        key = get_quantum_key_compatible(32)
        vote_data = {
            "voter_id": voter_id,
        ...
        BLOCKCHAIN.add_block({**vote_data, "vote_hash": vote_record["vote_hash"], "block_type": "vote"})
    ...
    return jsonify({"ok": True, "vote_id": vote_data["vote_id"]})
...
def get_results():
    ...
    } for i, p in enumerate(ranked)], "total_votes": tot})
...
if __name__=="__main__":
    print("🚀 Quantum-Blockchain Voting API running on :5000")
